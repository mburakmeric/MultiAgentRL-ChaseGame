"""
Training script for Chaser2 agent using Stable-Baselines3.
Can optionally load a trained Chaser1 model as opponent.
"""
...
def make_env(agent_name: str, rank: int, seed: int = 0, chaser1_model_path: str = None):
    """
...
def train_chaser2(args):
    ...
    if args.n_envs > 1:
        env = SubprocVecEnv([
            make_env(agent_name, i, args.seed, args.chaser1_model) 
            for i in range(args.n_envs)
        ])
    else:
        env = DummyVecEnv([make_env(agent_name, 0, args.seed, args.chaser1_model)])
    
    # Create evaluation environment
    eval_env = DummyVecEnv([make_env(agent_name, 99, args.seed, args.chaser1_model)])
...
def main():
    parser = argparse.ArgumentParser(description="Train Chaser2 agent")
    parser.add_argument(
        "--timesteps",
        type=int,
        default=500000,
